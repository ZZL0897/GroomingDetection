...
# This program only generates STimages of the one selected video
...
for frameInd in range(startFrame, endFrame, 1):
    ...
    maxMovement, cfrVectRec, tfrVectRec, frameVectFloatRec = getting_frame_record(frRec, 0, TimeWindow, fbList, newSize,
                                                                                  roi, CVNsize,
                                                                                  tarRec)

    im3C = create_ST_image(cfrVectRec, tfrVectRec, CVNsize)

    # Using plt to save as RGB, if using cv2, saving as BGR
    plt.imsave(r'G:\new\st' + '/' + str(frameInd) + '.jpg', im3C) # Input the save path
